      ...
      <div className="relative w-full max-w-4xl mx-auto">
        ...
        <div className="relative overflow-hidden shadow-card">
          ...
          transform: `translateX(-${currentIndex * 100}%)`
            ...
            {banners.map(banner => <div key={banner.id} className="aspect-[4/5] w-full flex-shrink-0">
        ...
        {/* Dots Indicator */}
        {banners.length > 1 && <div className="flex justify-center space-x-2 mt-4">
            {banners.map((_, index) => <button key={index} className={`w-3 h-3 rounded-full transition-colors duration-200 ${index === currentIndex ? 'bg-primary' : 'bg-muted-foreground/30 hover:bg-muted-foreground/50'}`} onClick={() => goToSlide(index)} />)}
          </div>}
      </div>
    </section>;
}